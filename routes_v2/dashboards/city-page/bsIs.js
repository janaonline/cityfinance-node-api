...
function createResponseStructure(yearWiseLedgerData, btnKey) {
	let getStructure = BALANCE_SHEET_STRUCTURE;
	if (btnKey === 'incomeStatement') getStructure = INCOME_STATEMENT_STRUCTURE;

	// Deep clone BS or IS STRUCTURE to avoid mutating shared reference
	const responseStructure = JSON.parse(JSON.stringify(getStructure));
