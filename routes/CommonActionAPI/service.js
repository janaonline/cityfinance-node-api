...
let objectFields = {
  waterSupply_actualIndicator: {
    fieldName: "actualYear",
    object: { ...yearValueField },
  },
...
var annualRadioButtons = {
  // if there are any label changes for radio button in frontend please update here
  Yes: true,
  No: false,
  Agree: true,
};
...
const calculateStatus = (status, actionTakenByRole, isDraft, formType) => {
  switch (formType) {
    case "ULB":
      switch (true) {
        case (status == "PENDING" || !status || "N/A") &&
...
module.exports.calculateStatusForFiscalRankingForms = (
  status = "",
  actionTakenByRole = "",
  isDraft = "",
  formType
...
module.exports.updateForm = async (req, res) => {
  try {
    ...
    const masterForm = await Sidemenu.findOne({
        ...
        if (masterForm.name == "Annual Accounts") {
          ...
          if (form.audited.submit_annual_accounts) {
            for (let key in form.audited.provisional_data) {
              if (
                typeof form.audited.provisional_data[key] == "object" &&
                form.audited.provisional_data[key] != null
              ) {
                if (form.audited.provisional_data[key]) {
                  if (actionTakenByRole === "STATE") {
                    form.audited.provisional_data[key]["status"] =
                      formData.status;
                    form.audited.provisional_data[key]["rejectReason_state"] =
                      formData.rejectReason_state;
                    form.audited.provisional_data[key]["responseFile_state"] =
                      formData.responseFile_state;
                  } else if (actionTakenByRole === "MoHUA") {
                    form.audited.provisional_data[key]["status"] =
                      formData.status;
                    form.audited.provisional_data[key]["rejectReason_mohua"] =
                      formData.rejectReason_mohua;
                    form.audited.provisional_data[key]["responseFile_mohua"] =
                      formData.responseFile_mohua;
                  }
                }
              }
            }
          ...
          if (form.unAudited.submit_annual_accounts) {
            for (let key in form.unAudited.provisional_data) {
              if (
                typeof form.unAudited.provisional_data[key] == "object" &&
                form.audited.provisional_data[key] != null
              ) {
                if (form.unAudited.provisional_data[key]) {
                  if (actionTakenByRole === "STATE") {
                    form.unAudited.provisional_data[key]["status"] =
                      formData.status;
                    form.unAudited.provisional_data[key]["rejectReason_state"] =
                      formData.rejectReason_state;
                    form.unAudited.provisional_data[key]["responseFile_state"] =
                      formData.responseFile_state;
                  } else if (actionTakenByRole === "MoHUA") {
                    form.unAudited.provisional_data[key]["status"] =
                      formData.status;
                    form.unAudited.provisional_data[key]["rejectReason_mohua"] =
                      formData.rejectReason_mohua;
                    form.unAudited.provisional_data[key]["responseFile_mohua"] =
                      formData.responseFile_mohua;
                  }
                }
              }
          ...
          if (form.unAudited) {
            if (actionTakenByRole === "STATE") {
              ...
              form.unAudited["rejectReason_state"] =
                formData.rejectReason_state;
              form.unAudited["responseFile_state"] =
                formData.responseFile_state;
            } else if (actionTakenByRole === "MoHUA") {
              form.unAudited["status"] = formData.status;
              form.unAudited["rejectReason_mohua"] =
                formData.rejectReason_mohua;
              form.unAudited["responseFile_mohua"] =
                formData.responseFile_mohua;
            }
          }
...
module.exports.annualaccount = catchAsync(async (req, res) => {
  ...
  const forms = await collection
    .find({ ulb: { $in: data.ulb }, [condition.design_year]: data.design_year })
    .lean();
  let form = {},
    numberOfFormsUpdated = 0;
  for (let i = 0; i < data.ulb.length; i++) {
...
module.exports.canTakeActionOrViewOnly = (
  data,
  userRole,
  adminLevel = false
) => {
  ...
  switch (true) {
    ...
    case status == StatusList.Under_Review_By_State && userRole == "STATE":
      return true;
      break;
    case status == StatusList.Under_Review_By_MoHUA &&
      adminLevel &&
      (userRole == "MoHUA" || userRole == "ADMIN"):
      console.log("adminglevel ::: ", adminLevel);
      return true;
      break;
    case status == StatusList.Under_Review_By_State &&
      (userRole == "MoHUA" || userRole == "ADMIN"):
      return false;
      break;
...
module.exports.canTakeActionOrViewOnlyMasterForm = (params) => {
  ...
  switch (true) {
    ...
    case status == MASTER_STATUS["In Progress"]:
      return false;
      break;
    case status == MASTER_STATUS["Under Review By State"] &&
      userRole == "STATE":
      return true;
      break;
    case status == MASTER_STATUS["Under Review By MoHUA"] &&
      adminLevel &&
      (userRole == "MoHUA" || userRole == "ADMIN"):
      console.log("adminglevel ::: ", adminLevel);
      return true;
...
class AggregationServices {
  ...
  static addMultipleFields(obj, arrayForm) {
    ...
    try {
      ...
      for (var field in obj) {
        ...
        returnable["$addFields"][fieldName] =
          type == "lakhs"
            ? this.convertIntoLakhs(field)
            : this.convertToCr(field);
        if (arrayForm) {
          let tempObj = { $addFields: {} };
          tempObj["$addFields"][fieldName] =
            type == "lakhs"
              ? this.convertIntoLakhs(field)
              : this.convertToCr(field);
          temp.push(tempObj);
  ...
  static addConvertedAmount(field, fieldName, type) {
    ...
    obj["$addFields"][fieldName] =
      type == "lakhs" ? this.convertIntoLakhs(field) : this.convertToCr(field);
    return obj;
  }
  ...
  static getCommonCurrencyConvertor(fieldName, arr, def) {
    ...
    obj["$switch"]["branches"].push(
      this.getCasesForCurrenCon(fieldName, arr[0], 1000, 10000)
    );
    obj["$switch"]["branches"].push(
      this.getCasesForCurrenCon(fieldName, arr[1], 10000, 1000000)
    );
    obj["$switch"]["branches"].push(
      this.getCasesForCurrenCon(fieldName, arr[2], 1000000, 100000000)
    );
    return obj;
  }
}
...
module.exports.getCurrentFinancialYear = () => {
  ...
  if (today.getMonth() + 1 <= 3) {
    fiscalyear =
      ...
      1 +
      "-" +
      today.toLocaleDateString("en", { year: "2-digit" });
  } else {
    fiscalyear =
      today.getFullYear() +
      "-" +
      (parseInt(today.toLocaleDateString("en", { year: "2-digit" })) + 1);
  }
  return fiscalyear;
};
...
async function decideValues(temp, shortKey, objects, req) {
  try {
    let service = new PayloadManager(
      temp,
      shortKey,
      objects,
      req,
      shortKeysWithModelName
    );
...
async function returnParsedObj(objects, req) {
  try {
    ...
    if (splittedShortKey.length > 1) {
      ...
      let obj = splittedShortKey.reduceRight(
        (obj, key) => ({ [key]: obj }),
        value
      );
      return obj;
...
function roleWiseJson(json, role) {
  ...
  try {
    ...
    json.question = json.question.filter(
      (item) => !removableObjects.includes(item.shortKey)
    );
    // }
  } catch (err) {
...
async function handleSelectCase(question, obj, flattedForm) {
  try {
    if (question.modelName) {
      ...
      let tempObj = question.answer_option.find(
        (item) => item.option_id.toString() == value.toString()
      );
      if (tempObj) {
...
module.exports.mutateJson = async (
  jsonFormat,
  keysToBeDeleted,
  query,
  role
...
async function handleArrOfObjects(question, flattedForm) {
  try {
        ...
        for (let keys in obj) {
          ...
          if (questionObj) {
            ...
            if (questionObj.isQuestionDisabled !== true) {
              questionObj.isQuestionDisabled = handleDisableFields({
                disableFields,
              });
            ...
            let modifiedObj = await handleRangeIfExists(
              { ...questionObj },
              obj
            );
    ...
    project_arr = project_arr.map((item) =>
...
async function appendChildQues(question, obj, flattedForm) {
  try {
    ...
    if (customShortKeys.includes(question.shortKey)) {
      let childQuestionData = await appendvalues(
        question.childQuestionData,
        flattedForm,
        question.shortKey,
        question
      );
...
const handleNumericCase = async (question, obj, flattedForm, mainKey) => {
  try {
    ...
    if (mainKey) {
      ...
      if (
        flattedForm[key] == undefined &&
        flattedForm[key + ".value"] == undefined
      ) {
        value = "";
      } else {
        value =
          flattedForm[key] === undefined
            ? flattedForm[key + ".value"]
            : flattedForm[key];
...
async function handleConsentCase(question, obj, flattedForm, mainKey) {
  try {
    ...
    if (Object.keys(flattedForm).includes(mainKey)) {
      ...
      let answer = question.answer_option.find(
        (item) => item._id === answerByBoolean
      );
...
const handleRadioButtonCase = async (question, obj, flattedForm, mainKey) => {
  try {
    ...
    let answerIds = question.answer_option.map((item) => ({
      [item.name]: item._id,
    }));
    ...
    if (mformValue) {
      let answerObj = question.answer_option.find(
        (item) => item.name === mformValue
      );
...
function handledateCase(question, obj, flattedForm) {
  try {
    ...
    if (flattedForm[mainKey] === undefined || flattedForm[mainKey] === null) {
      ...
      flattedForm[mainKey] = new Date(flattedForm[mainKey])
        .toISOString()
        .split("T")[0];
    }
    question["modelValue"] = flattedForm[mainKey];
...
function manageDisabledQues(question, flattedForm) {
  try {
    ...
    let mainKey = question.shortKey
      .split(".")
      .slice(0, question.shortKey.split(".").length - getValue)
      .join(".");
    actionKeys.forEach((item) => {
      let key = mainKey + "." + item;
      let keyItem = question.shortKey + "." + item;
      let included =
        Object.keys(flattedForm).includes(key) ||
        Object.keys(flattedForm).includes(keyItem);
      key = Object.keys(flattedForm).includes(key) ? key : keyItem;
      if (included) {
        question[item] = flattedForm[key];
        if (item === "status") {
          if (
            allowedStatuses.includes(flattedForm[key]) &&
            formType === "ULB"
          ) {
...
async function mutuateGetPayload(
  jsonFormat,
  flattedForm,
  keysToBeDeleted,
  role
  ...
  try {
    ...
    for (let key in obj) {
      ...
      if (questions) {
        ...
        let modifiedKeys = Object.keys(modifiedShortKeys);
        let modifiedObjects = questions.filter((item) =>
          modifiedKeys.includes(item.shortKey)
        );
      }
    }
    return obj;
...
async function handleCasesByInputType(question) {
  try {
    ...
    switch (question.input_type) {
      case "3":
        if (question.modelName) {
          obj = await appendAnswerOptions(
            question.modelName,
            question,
            question.modelFilter
          );
        }
        break;
      // case "11":
  ...
  try {
    ...
    documents.forEach((item, index) => {
      let answerObj = {
        name: item.name,
        did: [],
        _id: obj.answer_option
          ? findId(obj["answer_option"], item.name, index)
          : index.toString(),
        option_id: item._id,
        viewSequence: obj.answer_option
          ? findId(obj["answer_option"], item.name, index, "viewSequence")
          : (index + 1).toString(),
      };
      childObj = {
        type: item._id,
    ...
    obj["answer_option"] = answerOptions.sort(
      (a, b) => parseInt(a.viewSequence) - parseInt(b.viewSequence)
    );
...
module.exports.masterAction = async (req, res) => {
  try {
    ...
    let { formId, multi, shortKeys, responses, ulbs, form_level, design_year } =
      bodyData;

    if (
      !formId ||
      !bodyData.hasOwnProperty("multi") ||
      !shortKeys ||
      !responses ||
      !ulbs ||
      !ulbs.length ||
    ...
    for (let form of formData) {
      ...
      if (form_level === FORM_LEVEL["form"]) {
        ...
        if (multi) {
          ...
          if (
            Array.isArray(currentStatusShortKeys) &&
            !currentStatusShortKeys.length
          ) {
          ...
          currentStatusShortKeys = currentStatusShortKeys.map(
            (el) => el.shortKey
          );
          ...
          //gets tabs array
          let { outer: tabLevelShortKeys } = await getSeparatedShortKeys({
            shortKeys,
          });
          let tabShortKeyObj = {},
            tabShortKeyResponse = {};
          ...
          for (let response of responses) {
            response["status"] = Number(response["status"]);
            separator = response.shortKey.match(tabRegex)
              ? tabSeparator
              : dotSeparator;
            let splitedArrayTab =
            ...
            if (
              ...
              tabShortKeyObj[splitedArrayTab] =
                tabShortKeyObj[splitedArrayTab] + 1;
            }
...
async function saveStatus(params) {
  try {
    ...
    multi && [FORM_LEVEL["question"], FORM_LEVEL["tab"]].includes(form_level)
      ? (currentStatusData["shortKey"] = shortKey)
      : "";
    ...
    multi && [FORM_LEVEL["question"], FORM_LEVEL["tab"]].includes(form_level)
      ? (statusHistory["shortKey"] = shortKey)
      : "";
...
const getMasterAction = async (req, res) => {
  try {
    ...
    const model = require(`../../models/${path}`);
    const form = await model
      .findOne(condition, { _id: 1, currentFormStatus: 1 })
      .lean();
    if (!form) {
      return Response.BadRequest(res, {}, "No Form Found!");
    }
    let currentStatusResponse = await CurrentStatus.find({
      recordId: form._id,
    }).lean();
    ...
    if (formId === FORMIDs["AnnualAccount"]) {
      currentStatusResponse = filterStatusResponseTab(
        currentStatusResponse,
        currentFormStatus
      );
    } else {
      currentStatusResponse = filterStatusResponse(
        currentStatusResponse,
        currentFormStatus
      );
    }
    ...
    if (formId === FORMIDs["AnnualAccount"]) {
      ...
      currentStatusResponse = groupByKey(
        currentStatusResponse,
        "actionTakenByRole"
      );
    }
...
function getCurrentStatusTab(key, statuses) {
  if (key === "ULB") {
    statuses = statuses.filter((el) => {
      return (
        el.status <= MASTER_STATUS["Under Review By State"] &&
        el.status > MASTER_STATUS["Not Started"]
      );
    });
    ...
    statuses = statuses.filter((el) => {
      return (
        el.status < MASTER_STATUS["Submission Acknowledged By MoHUA"] &&
        el.status > MASTER_STATUS["Under Review By State"]
      );
    });
    return filterStatusForTab(statuses);
...
function filterStatusForTab(statuses) {
  ...
  statuses.forEach((el) => {
    ...
    if (
      statusObj &&
      ROLE_PRIORITY[statusObj.actionTakenByRole] <
        ROLE_PRIORITY[el.actionTakenByRole]
    ) {
      output.splice(idx, 1);
      output.push(el);
    } else if (!statusObj) {
...
function appendKeysForAA(currentStatusResponse) {
  const shortKeysToAppend = {
    ...
    "unAudited.inc_exp": [
      "unAudited.inc_exp.revenue",
      "unAudited.inc_exp.expense",
    ],
    "audited.inc_exp": ["audited.inc_exp.revenue", "audited.inc_exp.expense"],
  };
  currentStatusResponse = appendStatus(
    currentStatusResponse,
    shortKeysToAppend
  );
  return currentStatusResponse;
...
const handleChildValues = async (childObj, item, req) => {
  try {
    if (item && item.nestedAnswer) {
      if (
        Object.keys(arrFields).includes(item.shortKey) &&
        !Array.isArray(childObj[item.shortKey])
      ) {
...
function checkIfUlbHasAccess(ulbData, userYear) {
  try {
    ...
    let prevYear = `${(prevYearArr[0] - 1).toString().slice(-2)}${(
      prevYearArr[1] - 1
    )
      .toString()
      .slice(-2)}`;
...
module.exports.decideDisabledFields = (form, formType) => {
  ...
  if (form.status == "") {
    formStatus =
      MASTER_STATUS_ID[parseInt(form.currentFormStatus)] || "Not Started";
  }
