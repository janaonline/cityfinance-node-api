...
const { propertyTaxOpFormJson, skippableKeys, financialYearTableHeader, specialHeaders, skipLogicDependencies,childKeys,reverseLogicKeys } = require('./fydynemic')
...
function getLabelName(type) {
    try {
        let indicators = {
            ...
            ulbPassedResolPtax: 'Has the ULB passed the resolution for collecting property tax?',
...
const canShow = (key, results, updatedDatas,ulb) => {
    try {
        ...
        if (Object.keys(skippableKeys).includes(key)) {
            ...
            if(reverseLogicKeys.includes(key)){
