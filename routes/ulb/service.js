...
// Bulk Update censusCode, sbCode, population fields of All Ulbs
module.exports.bulkUpdate = async function (req, res) {
  try {
    let user = req.decoded
      // _id = ObjectId(req.params._id),
     const objArray = ULBSARRAY;
     if(objArray.length){
       ...
       if (actionAllowed.indexOf(user.role) > -1) {
         ...
         let condition = { code: obj['City Finance Code'] };
         let  newObj={};
         newObj["modifiedAt"] = new Date();
          newObj['censusCode'] = obj['Census Code'] ? obj['Census Code'] : null;
           newObj['sbCode'] = obj['Swatcha Bharat Code'] ? obj['Swatcha Bharat Code'] : null;
           newObj['population'] =  obj['Population'] ? obj['Population'] : 0;
         try {
