...
const calData = (data) =>{
 let copyData = []
  ...
  for(let el of data){
    ...
    if(el._id.code == "110" || el._id.code == "130" || el._id.code == "140" || el._id.code == "150" || el._id.code == "180"){
...
const revenue = catchAsync(async (req,res)=>{
  let {state, financialYear, headOfAccount, filterName, isPerCapita, ulbs, compareType, getQuery} = req.body
  ...
  _id: null,
    ...
    {
    $project:{
  average:{ $cond: [ { $eq: [ "$denominator", 0 ] }, 0, {"$divide":["$numerator", "$denominator"]} ] } 
                  ...
                  },
            {
                ...
                let perCapita = {
                  $project:{
                    ...
                    stateId:1,
                    state:1,
                    totalRevenue: { $cond: [ { $eq: [ "$population", 0 ] }, 0, {"$divide":["$totalRevenue", "$population"]} ] } ,
                    population:1
                  }
                }
    let ulb_query = []
