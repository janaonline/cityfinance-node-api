        ...
        let query = [
            ...
            { $unwind: "$state" },
            {
                $project: {
                    ...
                    ulbName: "$ulb.name",
                    ulbCode: "$ulb.code",
                    cert: "$history.cert.url",
                    otherDocs: "$history.otherDocs.url",
        ...
        let query = [
            ...
            { $unwind: "$state" },
            {
                $project: {
                    ...
                    ulbName: "$ulb.name",
                    ulbCode: "$ulb.code",
                    proof: "$history.proof.url",
                    rateCard: "$history.rateCard.url",
                    ptCollection: "$history.ptCollection.url",
                    responseFile_state: "$history.responseFile_state.url",
                    responseFile_mohua: "$history.responseFile_mohua.url"
                }
            }
        ...
        let query = [
            ...
            { $unwind: "$state" },
            {
                $project: {
                    ...
                    stateName: "$state.name",
                    stateCode: "$state.code",
                    comManual: "$history.comManual.url",
                    floorRate: "$history.floorRate.url",
                    stateNotification: "$history.stateNotification.url",
                    responseFile_mohua: "$history.responseFile_mohua.url",
                    responseFile_state: "$history.responseFile_state.url",
                }
        ...
        let query = [
            ...
            { $unwind: "$state" },
            {
                $project: {
                    ...
                    stateName: "$state.name",
                    stateCode: "$state.code",
                    million_tied: "$history.million_tied.pdfUrl",
                    nonmillion_tied: "$history.nonmillion_tied.pdfUrl",
                    nonmillion_untied: "$history.nonmillion_untied.pdfUrl"
                }
            }
        ]
        ...
        let query = [
            ...
            { $unwind: "$state" },
            {
                $project: {
                    ...
                    responseFile_state: "$history.responseFile_state.url",
                    ptCollection: "$history.ptCollection.url",
                    responseFile: "$history.responseFile.url",
        ...
        let query = [
            ...
            { $unwind: "$state" },
            {
                $project: {
                    ...
                    ulbName: "$ulb.name",
                    ulbCode: "$ulb.code",
                    garbageFreeCities: { $arrayElemAt: ["$history.solidWasteManagement.documents.garbageFreeCities.url", 0] },
                    waterSupplyCoverage: { $arrayElemAt: ["$history.solidWasteManagement.documents.waterSupplyCoverage.url", 0] },
                    waterBalancePlan: { $arrayElemAt: ["$history.millionPlusCities.documents.waterBalancePlan.url", 0] },
                    waterPotability: { $arrayElemAt: ["$history.waterPotability.documents.waterPotabilityPlan.url", 0] },
                    cityPlan: { $arrayElemAt: ["$history.millionPlusCities.documents.cityPlan.url", 0] },
                    serviceLevelPlan: { $arrayElemAt: ["$history.millionPlusCities.documents.serviceLevelPlan.url", 0] },
                    solidWastePlan: { $arrayElemAt: ["$history.millionPlusCities.documents.solidWastePlan.url", 0] }
                }
            }
        ]
        ...
        let query = [
            ...
            { $unwind: "$state" },
            {
                $project: {
                    ...
                    ulbName: "$ulb.name",
                    ulbCode: "$ulb.code",
                    documents_15_16_pdf: { $arrayElemAt: ["$history.documents.financial_year_2015_16.pdf.url", 0] },
                    documents_15_16_excel: { $arrayElemAt: ["$history.documents.financial_year_2015_16.excel.url", 0] },
                    documents_16_17_pdf: { $arrayElemAt: ["$history.documents.financial_year_2016_17.pdf.url", 0] },
                    documents_16_17_excel: { $arrayElemAt: ["$history.documents.financial_year_2016_17.excel.url", 0] },
                    documents_17_18_pdf: { $arrayElemAt: ["$history.documents.financial_year_2017_18.pdf.url", 0] },
                    documents_17_18_excel: { $arrayElemAt: ["$history.documents.financial_year_2017_18.excel.url", 0] },
                    documents_18_19_pdf: { $arrayElemAt: ["$history.documents.financial_year_2018_19.pdf.url", 0] },
                    documents_18_19_excel: { $arrayElemAt: ["$history.documents.financial_year_2018_19.excel.url", 0] },
                }
            }
        ]
